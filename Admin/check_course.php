<?php 
	...
	if(empty($coursename) || empty($courseinfo) || empty($credit) || empty($classtime) || empty($teacher) || empty($limitpeople)){
		echo "<script>alert('不允许为空，请重新输入!');location='../tea_course.php';</script>";
	}else{
			$user = new Model("course_info");
			if($user -> insert($post)){
				echo "<script>alert('添加课程成功!\(^o^)/');location='../tea_course_all.php';</script>";
			}else{
				echo mysql_error();
				echo "<script>alert('添加课程失败失败!~~~~(>_<)~~~~ ');location='../tea_course.php';</script>";
			}
		}
